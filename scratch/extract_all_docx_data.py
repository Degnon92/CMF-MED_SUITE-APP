...
def parse_french_date(text):
    # Match: "le 15 janvier 2025" or "le 15/01/2025" or "15-01-2025"
...
# Filter administrative headers in sidebars
...
for idx, report in enumerate(raw_reports):
    ...
    # Strip age from name header if present
    clean_name = raw_name
    age_in_name = None
    ...
    if len(words) < 2:
        # Lacks a first name
        # We shouldn't reach this because we verified there are 0 patients like this
        continue
    ...
    # Clean diagnosis: strip trailing punctuation and spaces
    ...
    if not has_insurance:
        if any(w in full_text.lower() for w in ["assurance", "mutuelle", "prise en charge", "sinistre", "bon"]):
            insurer = "ASCOMA" # default insurance fallback
            prise_en_charge = 80
    ...
    if not report_date:
        report_date = "2025-02-15" # Default reasonable fallback date
    ...
    for line in clean_lines[:4]:
        ...
        if "COMPTE RENDU D'HOSPITALISATION" in line_upper or "COMPTE-RENDU D'HOSPITALISATION" in line_upper:
            template_id = "rapport_hospi_simple"
            doc_title = "RAPPORT D'HOSPITALISATION"
            doc_category = "Rapport HOSPI"
            break
        elif "CERTIFICAT DE REPOS" in line_upper:
    ...
    if clean_name not in patients:
        ...
        # Merge clinical details if first entry was empty
    ...
    # Add to clinical documents
    doc_id = f"DOC-CMF-{idx+1:03d}"
...
# Output stats
print(f"Total processed reports: {len(raw_reports)}")
print(f"Unique extracted patients: {len(patients)}")
print(f"Unique diagnoses: {len(all_diagnoses)}")
print(f"Unique interventions: {len(all_interventions)}")

# Write to temp files for validation
with open("patients_extracted.json", "w", encoding="utf-8") as f:
...
print("Saved intermediate extraction results to patients_extracted.json and docs_extracted.json.")
